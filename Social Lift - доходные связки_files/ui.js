...
function AjaxPage(href) {
	...
	AjaxRequest('GET', url, {}, function (data) {
		...
		ZeroClipboardCreate('.kmfdm-page__body .kmfdm-page__main .clipboard-btn,.kmfdm-page__body .kmfdm-page__main .js-copy,[data-clipboard-text]');
		try {
...
function AsOverlayRequest(url, data, href) {
	AjaxRequest('GET', url, data || {}, function (data) {
		...
		MY_OVERLAY.active(html, function () {
			...
			initWidget('#overlay-body widget[data-widget]');
		}, function () {
		...
		preConfirm: async (inputValue) => {
			...
			if (callback) {
				return (new Promise((resolve, reject) => {
					//debugger;
					return callback(inputValue, {resolve: resolve, reject: reject});
				})).then(() => {
					//debugger;
					...
					let errorMessage = typeof error.responseText !== 'undefined' ? error.responseText : error.error_message;
				//	debugger;
					SweetAlert.showValidationMessage(errorMessage)
					cancelCallback();
					return false
				});
			}
			...
			return (new Promise((resolve, reject) => {
				...
				AjaxRequest('POST', '/api/secutiry/check-query-code', {code: inputValue}, function (data) {
					window.check2FaSubmiting = null;
					$("body").removeClass("load");
					if (data.result == "success") {
...
function copied(text) {
	...
	$('body').append(
		'<div class="zero-copyed show">' + (/^http[s]?:\/\//.test(text) ? $GL('copyed_link:Ссылка скопирована') : $GL('copyed:Скопировано')) + '</div>');
...
(function ($) {
	...
	}).on('click', '[data-reflink-generator-params]', function (e) {
		e.preventDefault();
		let _this = $(e.target);
		if (_this.data('clipboard-text') !== undefined || _this.hasClass('zerocpSet')) {
			return;
		}

		AjaxRequest('POST', '/api/component/short-ref-generator', {data: _this.data('reflink-generator-params')}, function (response) {

				let data = response.data;
				_this.attr('data-clipboard-text', data.link);

				/*let p = new Promise(() => {
					debugger;
					if (!navigator.clipboard) {
						if (fallbackCopyTextToClipboard(data.link)) {
							resolve();
						} else {
							reject();
						}
				...
				p.then(() => {
					...
					SweetAlert.fire({'title': 'Ваша ссылка', 'html': data.link + '<i class="zmdi zmdi-copy" data-clipboard-text="' + data.link + '"></i>'});
					ZeroClipboardCreate('[data-clipboard-text]');
				});*/

				/**/
				let cp = new ClipboardJS(_this.get(0), {
	...
	*/
	...
	$(document).ready(() => {
		if (typeof Sweetalert2 === 'function') {
			window.Swal = function (params, callback) {
				var functionAsStr = String(callback).replace(/\s/g, '');
				...
				let _callback = function (res) {
					...
					if (typeof res != "Object" || res.isConfirmed || functionHandlesCancel) {
						res = callback(typeof res != "Object" ? res : (typeof res.value !== 'undefined' ? res.value : res.isConfirmed == true));
						if (res == false) {
							return $('.swal2-validation-message').text();
						}
					}
				}
				...
				if (params.type == 'input') {
					params.input = 'text';
					params.inputValidator = _callback;
				}

				let sw = new window.Sweetalert2(params)
				if (callback && typeof params.input == "undefined") {
					sw.then(_callback)
				}
			}
			...
			swal.showInputError = function (message) {
				$('.swal2-validation-message').text(message).show()
			}
			window.Sweetalert = {
				fire: function (params) {
					return new Sweetalert2(params);
				}
			}
		}
	});
