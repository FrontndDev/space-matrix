	...
	(function () /** ajax settings */ {
			...
			if (typeof data.responseJSON !== 'undefined') {
				...
				if (data.status === 422) {
					swal({
						...
						text: typeof data.responseJSON.error_details === 'srting' ? data.responseJSON.error_details : Object.values(
							data.responseJSON.error_details).join('<br>'),
					});
		...
		$.ajaxSetup({
			beforeSend: function (jqXHR, settings) {
				...
				let fpid = window.localStorage.getItem('_fp');
				if (fpid !== null) {
					jqXHR.setRequestHeader("X-FP", btoa(fpid));
				}
			...
			complete: function (jqXHR, settings) {
				...
				if (typeof jqXHR.responseJSON !== 'undefined') {
					if (jqXHR.responseJSON.result !== undefined && jqXHR.responseJSON.result === 'error') {
						let text = jqXHR.responseJSON.error_code + ' # ';
						if (typeof jqXHR.responseJSON.error_details !== 'undefined') {
							text += typeof jqXHR.responseJSON.error_details === 'string' ? jqXHR.responseJSON.error_details : Object.values(
								jqXHR.responseJSON.error_details).pop();
						} else {
							text += jqXHR.responseJSON.error_description;
							if (typeof jqXHR.responseJSON.error_message !== 'undefined') {
								text += '<br>' + jqXHR.responseJSON.error_message;
							}
						}
						show_message('red', text);
						...
						if (typeof jqXHR.responseJSON.error_message === 'Object') {
							...
							if (typeof jqXHR.responseJSON.swal === 'undefined') {
								show_message('red',
									(jqXHR.responseJSON.error !== 0 && jqXHR.responseJSON.error !== undefined ? "#" + jqXHR.responseJSON.error + "  " : '') + (jqXHR.responseJSON.user_message || jqXHR.responseJSON.error_message),
									$GL('error'))
								return;
							}
		...
		$(document).ready(function () {
			...
			function sendActionCode($form) {
				debugger;
				var $confirmAction = $form.find('input[name=confirmAction]');
				var $confirmActionCode = $form.find('input[name=confirmActionCode]');

				AjaxRequest('POST', '/api/security/init-action-session', {actionName: $confirmAction.val()}, function (response) {
			...
			$('body:not(.binded)')
				...
				.on('submit', 'form[target=space_saveform]', function (e) {
					...
					if ($form.data('canSubmit') !== true) {
						...
						AjaxRequest('POST', "/api/security/init-action-session", {actionName: $confirmAction.val()}, function (data) {
								if (data.result !== 'success') {
									return;
								}
								fpLoad();
...
class SpaceSocket {
	...
	constructor(room, incomeMessageCallback) {
		...
		this.incomeMessage = incomeMessageCallback || function (message) {console.log(message)}
		this.startSocket();
	}
...
function fpLoad() {
	if (window.localStorage.getItem('_fp') !== null) {
		return;
	}
	try {
		// let fp = import('https://fpcdn.io/v3/' + window.fpt)
		var fp = import( window.fpt !== '' ? 'https ://halk.ai/HE6epenf0RENDdhC/2SRN4NZxtlfIzrMh?apiKey=' + window.fpt : 'https://openfpcdn.io/fingerprintjs/v4')
			.then(FingerprintJS => FingerprintJS.load({
				endpoint: [
					"https://halk.ai/HE6epenf0RENDdhC/puHcgqRetyFJa2RQ?region=eu",
		...
		fp.then(result => {
			...
			$('#fp').val(visitorId);
			$.cookie('_fp', visitorId);
			window.localStorage.setItem('_fp', visitorId)
		})
