var lmdd = (function () {
	...
	var scrollManager = {
		...
		get speed() {
			return Math.max(
				0, this.sm + (Math.max(0 - this.event.clientY, 0 - this.event.clientX, this.event.clientY - this.el.clientHeight, this.event.clientX - this.el.clientWidth)));
		},
	...
	function simulateMouseEvent(event) {//convert touch to mouse events
		...
		var eventTarget = (event.type === "touchmove") ? document.elementFromPoint(simulatedEvent.clientX,
			simulatedEvent.clientY) || document.body : event.target;
		if (status === "dragStart") {
	...
	function updateCurrentContainer() {
		...
		if (positions.currentTarget !== lastEvent.target) {
			...
			positions.currentContainer = getWrapper(lastEvent.target, scope.lmddOptions.containerClass, scope.lmddOptions.containerClass);
		}
	}
	...
	function acceptDrop(container, item) {
		var level = parseInt(container.dataset.level) || 1;
		if (scope.lmddOptions.maxLevel != -1 && (level >= scope.lmddOptions.maxLevel || (level > 1 && item.querySelector('.nested-item')))) {
			return false;
		}
	...
	function setElements(el) {//set animated and cloned elements
		...
		createRectRefs(scope);
		if (getComputedStyle(scope.cloneRef.parentElement, null).position === 'absolute') {
			var position = $(el).position();
			scope.cloneRef.style.top = position.top + "px";
			scope.cloneRef.style.left = position.left + "px";
		}
		animate(scope);
	...
	function killEvent() {//end current drag event
		...
		if (typeof dragged !== 'defined' && dragged !== null) {
			var parent = dragged.parentElement.closest('.' + scope.lmddOptions.containerClass);
			var level = parent ? parseInt(parent.dataset.level) : 1;
			...
			if (cloning) {
				clone.elref.dataset.level = level;
			} else {
				var oldLevel = parseInt(dragged.dataset.level);
				dragged.dataset.level = level;
				var child = dragged.querySelector('.' + scope.lmddOptions.containerClass);
				if (child) {
	...
	return {//exposed methods
		set: function (el, lmddOptions) {
			if (!el.lmdd) {
				...
				if (typeof el.lmddOptions.dragEnd === 'function') {
					tasks.onDragEnd.push(el.lmddOptions.dragEnd);
				}
