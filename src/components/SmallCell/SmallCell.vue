<template>
  <div
      class="small-cell"
      :class="[props.type, { 'skeleton-loader': !props.cell }]"
  >
    <template v-if="props.cell">
      <div
...
const selectedTypeTitle: ComputedRef<string> = computed(() =>
    store.state.listOfTypes?.types
        .find((type: Type) => props.cell && 'type' in props.cell && props.cell.type === type.type)?.title
)

const matrixTypeIsDreamTon9: ComputedRef<boolean> = computed(() =>
    !!props.cell && props.cell && 'type' in props.cell && props.cell.type === 'dream-ton_9'
)

const time = computed(() => {
  if (props.cell && !('time_to_activate' in props.cell)) {
    return ''
  }
