<template>
  <div class="partner-cell" :class="props.size">
    <div class="partner-cell__avatar" :class="props.type">
      <img :src="props.ceil?.matrix?.owner.photo" alt="Avatar" @click="$emit('open-m-matrix-partner')">
      <Reward
    ...
    <div class="partner-cell__id">
      ...
      {{ props.ceil?.matrix?.owner.id }}
    </div>

    <PartnerType
        :binstatus="props.ceil.matrix.binstatus[1]"
        :cellType="props.cellType"
        :type="props.type"
        v-if="props.ceil?.matrix"
    />
