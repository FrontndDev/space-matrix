<template>
  <div class="partner-cell" :class="[props.size, props.state, props.type]">
    <div
        class="partner-cell__avatar"
        :class="[props.state, props.type]"
        @click="$emit('open-m-matrix-partner')"
    >
      <img alt="Avatar" src="https://static.halk.ai/images/sex_0.jpg">

      <CellInformation :class="size"/>
    </div>
    <template v-if="!props.size">
      <div
          ...
          @click="$emit('open-m-matrix-partner')"
      >owner.fio</div>
      <div class="partner-cell__id" @click="">
