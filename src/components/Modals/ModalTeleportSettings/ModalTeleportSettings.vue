    ...
    <transition name="modal" class="modal">
      <div class="modal__container">
        <div class="modal__content">
          <div class="modal-teleport-settings">
            <ModalHeader @close-modal="$emit('close-modal')">Настройки телепорта</ModalHeader>
            <div class="modal-teleport-settings__container">
              <div class="modal-teleport-settings__content">
                <div class="modal-teleport-settings__prompt">
                  ...
                  <p>Ваш куратор {{ props.teleportCheckbox ? 'может' : 'не может' }} использовать <br> телепорт новичка на вас</p>
                </div>
                <ChainsButton class="w-100" @click="setValue">
                  <span>{{ props.teleportCheckbox ? 'Запретить' : 'Разрешить' }}</span>
                </ChainsButton>
              </div>
            </div>
          </div>
...
const emit = defineEmits(['close-modal', 'set-teleport-checkbox-value'])

const setValue = () => {
  emit('set-teleport-checkbox-value', !props.teleportCheckbox, 6)
  emit('close-modal')
}
</script>

<style scoped lang="scss">
@import "modalTeleportSettings";
</style>